...
├── .env.prod                       ← Production variables (CI/CD updates)
...
- ✅ **.env.prod** - Production (CI/CD managed)
...
- ✅ **docker-release.yml**
  ...
  - Updates .env.prod with new digests
...
- Updates image digests in .env.prod
...
- [x] Environment files (.env.local, .env.prod)
