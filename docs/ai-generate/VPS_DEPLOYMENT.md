...
After copying files from CI/CD, `.env.prod` contains:
...
3. **Updates `.env.prod`** with new digests
...
1. Edit `.env.prod` with old digest
...
| Rollback version | ❌ Update `.env.prod` from local, push |
