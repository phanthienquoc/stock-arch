...
def create_tp_sl(order_data: dict):
    """Create TP/SL orders"""
    # TODO: Implement TP/SL creation logic
    logger.info(f"Creating TP/SL for order: {order_data}")
    return True
...
def consume_filled_orders():
    ...
    try:
        ...
        logger.info("TP/SL Engine Service started, listening for filled orders...")
            ...
            if messages:
                for stream, group_messages in messages:
                    for msg_id, data in group_messages:
                        logger.info(f"Creating TP/SL for order: {data}")
                        create_tp_sl(data)
